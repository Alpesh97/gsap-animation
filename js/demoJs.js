...
function showOverlay(overlay, onComplete) {
  ...
  if (activeOverlay) { //if there's already an active overlay, hide it first.
    gsap.to(activeOverlay, {duration:0.25, rotationX:70, y:-300, z:-500, autoAlpha:0, display:"none", ease:"power1.in", onComplete:function() {
      if (overlayZIndex) {
  ...
  gsap.set(activeOverlay, {zIndex:3000});
  gsap.to("#overlay-dimmer", {duration:0.25, autoAlpha:0.6, ease:"none"});
  gsap.set(activeOverlay, {overflow:"auto", scale:0.5, rotationX:70, autoAlpha:0, y:-300, z:-500, xPercent:-50, yPercent:-50, x:0, transformPerspective:600, display:"block"});
...
function hideOverlay() {
  ...
  if (activeOverlay) {
    ...
    gsap.to(activeOverlay, {duration:0.25, autoAlpha:0, display:"none", onComplete:function() {
      if (overlayZIndex) {
        activeOverlay.style.zIndex = overlayZIndex;
      } else {
        gsap.set(activeOverlay, {clearProps:"zIndex"});
